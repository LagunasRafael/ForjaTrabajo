...
export const SettingsPage = () => {
  ...
  const [config, setConfig] = useState({
    siteName: 'Forja Trabajo',
    supportEmail: 'soporte@forjatrabajo.com',
    // 🟢 AQUÍ ESTÁ LA MAGIA: Leemos el disco en lugar de poner 'false' por defecto
    maintenanceMode: JSON.parse(localStorage.getItem('maintenance_mode') || 'false'),
    commissionRate: 10, 
  });
  ...
  const handleSaveGeneral = async (e: React.FormEvent) => {
    ...
    // 1. 💾 GUARDAMOS EN LOCALSTORAGE AL HACER CLIC EN GUARDAR
    localStorage.setItem('maintenance_mode', JSON.stringify(config.maintenanceMode));
    
    // 2. 📢 GRITAMOS A TODA LA APP QUE HUBO UN CAMBIO
    ...
    setTimeout(() => {
      ...
      if (config.maintenanceMode) {
        toast.warning('Modo mantenimiento activado. Los usuarios verán la app cerrada.');
      } else {
        toast.success('Configuración general actualizada y plataforma activa.');
      }
    }, 800);
  };
  ...
  // Dentro de SettingsPage.tsx
    ...
    // 🔥 ESTA ES LA LÍNEA NUEVA: Gritamos a toda la app que algo cambió
    window.dispatchEvent(new Event('maintenance_changed'));
